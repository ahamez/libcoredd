...
TEST(variant, unary_visitor)
{
  ...
  using variant_type = variant<double, char, int, long, foo>;

  {
    variant_type v((construct<double>()));
    ASSERT_EQ(0u, apply_visitor(visitor(), v));
  }
  {
    variant_type v((construct<char>()));
    ASSERT_EQ(1u, apply_visitor(visitor(), v));
  }
  {
    variant_type v((construct<int>()));
    ASSERT_EQ(2u, apply_visitor(visitor(), v));
  }
  {
    variant_type v((construct<long>()));
    ASSERT_EQ(3u, apply_visitor(visitor(), v));
  }
  {
    variant_type v(construct<foo>(), 42);
...
TEST(variant, binary_visitor)
{
  ...
  using variant_type = variant<double, int, foo>;

  {
    variant_type v1((construct<double>()));
    variant_type v2((construct<double>()));
    ASSERT_EQ(0u, apply_binary_visitor(visitor(), v1, v2));
  }
  {
    variant_type v1((construct<double>()));
    variant_type v2((construct<int>()));
    ASSERT_EQ(1u, apply_binary_visitor(visitor(), v1, v2));
  }
  {
    variant_type v1((construct<double>()));
    variant_type v2(construct<foo>(), 42);
    ASSERT_EQ(2u, apply_binary_visitor(visitor(), v1, v2));
  }
  {
    variant_type v1((construct<int>()));
    variant_type v2((construct<int>()));
  ...
  {
    variant_type v1(construct<foo>(), 42);
    variant_type v2(construct<foo>(), 33);
...
{
  {
    using variant_type = variant<bar, char, foo>;
    ASSERT_LE(8192 * sizeof(size_t), sizeof(variant_type));
    variant_type v((construct<foo>()));
    apply_visitor(visitor1(), v);
  }
  {
    using variant_type = variant<char, bar, foo>;
    ASSERT_LE(8192 * sizeof(size_t), sizeof(variant_type));
    variant_type v((construct<foo>()));
    apply_visitor(visitor2(), v);
  }
  {
    using variant_type = variant<foo, char, bar>;
    ASSERT_LE(8192 * sizeof(size_t), sizeof(variant_type));
    variant_type v((construct<foo>()));
